...
void ECVirtualMemory::AccessPage(int page){
    ...
    if (page_set.count(page) == 0){faults += 1;} //add to the back since there are no instances of the page 

    //before adding we have to check that we have space
    if(page_set.size() < max_num_pages){   //in the case where there is space
        page_list.push_back(page);      //add to the end 
        page_set.insert(page);
    }
    else{                                               //in the case where there isn't space 

        int lru_page = page_list.front(); //get the actual value so we can erase from the set
        page_list.erase(page_list.begin());            //take it out of the vector 
